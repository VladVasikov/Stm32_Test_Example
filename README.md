...
Для упрощения структура сигнала представляет массив с количеством импульсов и их длительностью периода(период уже рассчитан в отсчётах таймера).
...
Сгенерировать меандр необходимо **аппаратно** на выходе PA8 МК.
5. При запуске генератора с такими параметрами:
```
std::array<Packet,3> signal{{{10, 1000},{5, 2000},{20, 500}}};
generator.add(signal);
generator.start();
```
На выходе должен получиться меандр с общим количеством импульсов 35 штук, где сперва идут 10 с частотой 1000 Гц, затем 5 с частотой 500 Гц и 20 с частотой 2000 Гц (при условии что таймер тактируется на частоте 1МГц).
